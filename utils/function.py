...
def create_mask_from_lengths(lengths: Tensor, max_len: int = None):
    '''
    通过lengths数组创建mask
    lengths:(batch,)    batch内每个样本的seq_len
...
def statisticEvaluation(spo_list_pred, spo_list_true):
    import pandas as pd
    # 重新编写 evaluation 代码
    spo_count_pred = []
    for spos in spo_list_pred:
        spo_count_pred.append(len(spos))
    spo_count_true = []
    for spos in spo_list_true:
        spo_count_true.append(len(spos))
    pd.DataFrame([spo_count_pred, spo_count_true]).to_excel(dirname(dirname(__file__))+'\\data\\NYT-partial\\spo_count.xlsx')
    print('done')
...
def save_spo_list(dev_data: list, spo_list_pred: list, spo_list_true: list, file_path: str):
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        ...
        ujson.dump(sentences, f, ensure_ascii=False, indent=4)
        # text = ujson.dumps({
        #     'text': data['text'],
        #     'spo_true': [spo for spo in spo_true],
        #     'spo_pred': [spo for spo in spo_pred],
