...
class SelfAttentionEncoder(nn.Module):
    def __init__(self, embedding_dim: int, num_heads: int, num_layers: int, dropout_prob: float = 0.0,
                 device: str = 'cuda'):
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, input_embedding: Tensor, position_embedding: Tensor, mask: Tensor):
        ...
        # input_lengths_cpu:(batch_size,), 表示一个batch中各个样本的seq_len
        input_lengths_cpu = torch.sum(mask, dim=1).cpu()

        # 告诉RNN每个句子长度，RNN就不会计算pad掉的字符
        inputs = pack_padded_sequence(input=input_embedding, lengths=input_lengths_cpu, batch_first=True,
