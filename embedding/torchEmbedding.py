...
CHAR2ID_FILE = parent_path + dataset + '/char2id.json'
# bert vocab
...
class TorchEmbedding(nn.Module):
    def __init__(self, embedding_size: int, device: str = 'cuda', char2id_file: str = CHAR2ID_FILE,
                 from_pretrain: str = None):
        '''
        ...
        if from_pretrain is not None:
            embedding_init = np.load(from_pretrain).astype(np.float32)
            embedding_init = torch.FloatTensor(embedding_init)
            # freeze=True时，不训练嵌入矩阵
    ...
    def tokenize(self, text: list, max_len: int = None):
        ...
        # [pad]: 0, [unk]: 1,
        # 每个句子的长度
        length = np.array([len(text.strip().split(' ')) for text in text], dtype=np.int64)
        batch_size = len(text)
        ...
        # 因为python点操作很慢，所以把函数当做对象先拿出来
        get = self.char2id.get

        for i in range(batch_size):
            for j, word in enumerate(text[i].strip().split(' ')):
                text_id[i][j] = get(word, 1)  # 没有找到字符返回1，1是unk
        # ndarray转tensor，to('GPU')
